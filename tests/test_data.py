...
from src.data import ImageManifestDataset, build_resnet_transforms, load_class_names
...
class DataMappingTests(unittest.TestCase):
    ...
    def test_dataset_uses_global_manifest_mapping_not_visible_class_order(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            image_a = root / "a.jpg"
            image_b = root / "b.jpg"
            ...
            with manifest.open("w", newline="", encoding="utf-8") as handle:
                ...
                writer.writeheader()
                writer.writerow(
                    {"filepath": str(image_a), "label": 0, "class_name": "zebra", "split": "train"}
                )
                writer.writerow(
                    {"filepath": str(image_b), "label": 1, "class_name": "antelope", "split": "val"}
                )

            dataset = ImageManifestDataset(manifest, split="train")
            self.assertEqual(dataset.idx_to_class, {0: "zebra", 1: "antelope"})
            self.assertEqual(dataset.classes, ["zebra", "antelope"])
