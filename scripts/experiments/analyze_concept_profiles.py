"""Analyze AwA2 semantic concept profiles and prediction transitions."""
...
LOGGER = logging.getLogger("analyze_concept_profiles")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Optional stress-metrics CSV with original_prediction and perturbed_prediction columns.",
    )
    parser.add_argument(
...
def read_prediction_transitions(csv_path: Path) -> list[tuple[str, str, str]]:
    """Read changed predictions once per image and perturbation from a stress-metrics CSV."""
    ...
    with csv_path.open("r", newline="", encoding="utf-8") as handle:
        ...
        required = {"index", "original_prediction", "perturbed_prediction", "perturbation"}
        ...
        for row in reader:
            ...
            if source == target:
                continue
            key = (row["index"], row["perturbation"], source, target)
            if key in seen:
                continue
            seen.add(key)
...
def main() -> None:
    ...
    LOGGER.info(
        "Concept profile analysis complete: class_profiles=%s transitions=%s",
